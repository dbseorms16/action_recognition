...
import cv2
from functional import clipsampler
...
def inference_flow(model,
    ...
    for test_id in range(0, len(filenames) - 1):
        ...
        flow_pr = F.interpolate(flow_pr, size=224, mode='bilinear',
                                    align_corners=True)
        flow_pr[:, 0] = flow_pr[:, 0] * ori_size[-1] / inference_size[-1]
        flow_pr[:, 1] = flow_pr[:, 1] * ori_size[-2] / inference_size[-2]
       ...
       # print(flow_sum.shape)
        ...
        if inference_video is not None and save_video:
          ...
          #  if count <=latent_frame :                                           ## clip weighting
#             flow_sum+=np.sqrt(flow[:,:,0]**2 + flow[:,:,1]**2)
 ...
 #            print(flow_sum)
            ...
            # else :
            #     flow_sum/=(4*latent_frame)
                
            #     # print(flow_sum)
            #     cv2.imshow("s",flow_sum)

            #     cv2.waitKey()    
            #     cv2.destroyAllWindows()
            #     count = 1  
            #     flow_sum=np.zeros((flow.shape[0],flow.shape[1]))


            # if count%TUBE_SIZE != 0 :
            #     patch_sum+=(np.sqrt(flow[:,:,0]**2 + flow[:,:,1]**2))
            ...
            #     patch_sum/=(4*TUBE_SIZE)
            #     print(patch_sum)
            #     cv2.imshow("kkk",patch_sum)
            ...
            vis_flow_preds.append(np.float32(np.sqrt(flow[:,:,0]**2 + flow[:,:,1]**2)))
            #print(np.uint8(vis_flow_preds))
           # print(flow_frame(vis_flow_preds,4))
            #print((vis_flow_preds[0]))
           # print(len(vis_flow_preds[0][0]))
   ...
   # print(vis_flow_preds)
    print(clipsampler(vis_flow_preds))
    #print(np.shape(frame_attn(vis_flow_preds,3)))
   # print(np.shape(patch_attn(vis_flow_preds,3,9)))
    print('Done!')
